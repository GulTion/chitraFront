...
class Canvas extends Component {
  ...
  componentWillReceiveProps (nextProps) {
    ...
    if (nextProps.lines && nextProps.lines.length > this.state.lastLength) {
      for (let i = this.state.lastLength; i < nextProps.lines.length; i += 1) {
        ...
        this.draw(l.from.x, l.from.y, l.to.x, l.to.y, l.brushColor, l.lineWidth);
      }
  ...
  draw(lX, lY, cX, cY,cI,wI) {
    
    this.ctx.strokeStyle = cI;
  ...
  handleOnMouseMove = (e) => {
    if (this.drawing) {
      ...
      this.draw(this.lastX, this.lastY, currentX, currentY, this.props.brushColor,this.props.lineWidth);
      this.update();
